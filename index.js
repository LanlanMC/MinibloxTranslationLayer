...
const server = mc.createServer({
	'online-mode': false,
	motd: '\u00a76' + ' '.repeat(14) + 'Miniblox \u8f6c\u8bd1\u5c42 \u00a7c[1.8.9]\n\u00a7a',
...
function uuid(requeue) {
	if (!requeue) {
		currentUUID = '10000000-1000-4000-8000-100000000000'.replace(/[018]/g, j => (j ^ crypto.getRandomValues(new Uint8Array(1))[0] & 15 >> j / 4).toString(16));
	}
	console.log(`\x1b[32m[+] \u4f7f\u7528 UUID: ${currentUUID}\x1b[0m`)  // 使用 UUID: ${currentUUID}
	return currentUUID;
}
...
async function connect(client, requeue, gamemode, code) {
	...
	if (!fetched.ok) {
		...
		client.end(text ?? (fetched.statusText ?? '\u65ad\u5f00\u8fde\u63a5')); // 断开连接
		return;
	}

	fetched = await fetched.json();
	console.log(`\x1b[36m[*] \u8fde\u63a5\u81f3 ${fetched.serverId}\x1b[0m`);
	...
	ClientSocket.once('connect', () => {
		ClientSocket.sendPacket(new SPacketLoginStart({
			...
			session: session,
			hydration: '0',
			metricsId: uuid(requeue),
			clientVersion: VERSION
		}));
	});
	ClientSocket.once('CPacketJoinGame', packet => {
		if (!packet.canConnect) {
			client.end(packet.errorMessage ?? '\u65ad\u5f00\u8fde\u63a5'); // 断开连接
			return;
		}
	...
	ClientSocket.socket.io.on('reconnect_failed', () => {
		client.end('\u65e0\u6cd5\u8fde\u63a5\u81f3\u670d\u52a1\u5668\u3002'); // 无法连接至服务器。
	});
	ClientSocket.on('disconnect', reason => {
		if (skipKick > Date.now()) return;
		console.log(`\x1b[31m[-] \u8fde\u63a5\u5df2\u4e22\u5931: ${reason}\x1b[0m`) // 连接已丢失: ${reason}
		client.end(reason);
	});
